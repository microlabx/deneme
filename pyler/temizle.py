...
async def fastpurger(purg):
    """ .temizle komutu hedeflenen yanıttan başlayarak tüm mesajları temizler. """
    ...
    if purg.reply_to_msg_id is not None:
        ...
        await purg.edit("`Temizlemeye başlamak için bir mesaja ihtiyacım var. CeteUserBot`")
        return
    ...
    if BOTLOG:
        await purg.client.send_message(
            BOTLOG_CHATID,
            "Hedeflenen " + str(count) + " mesaj başarıyla silindi. CeteUserBot")
    await sleep(2)
    await done.delete()
...
async def purgeme(delme):
    ...
    smsg = await delme.client.send_message(
        delme.chat_id,
        "`Temizlik tamamlandı` " + str(count) + " tane mesaj silindi. CeteUserBot",
    )
    if BOTLOG:
