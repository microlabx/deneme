...
@register(outgoing=True, pattern="^.geçmiş(?: |$)(.*)")
async def _(event):
    if event.fwd_from:
        return 
    if not event.reply_to_msg_id:
       await event.edit("`Herhangi bir kullanıcı mesajına cevap verin.CeteUserBot`")
       return
    reply_message = await event.get_reply_message() 
    if not reply_message.text:
       await event.edit("`Mesaja cevap verin.CeteUserBot`")
       return
    chat = "@SangMataInfo_bot"
    sender = reply_message.sender
    if reply_message.sender.bot:
       await event.edit("`Botlara cevap veremezsiniz.CeteUserBot`")
       return
    await event.edit("`İşleniyor...CeteUserBot`")
    async with bot.conversation(chat, exclusive=False) as conv:
          ...
          if not response:
              await event.edit("`Botdan cevap alamadım!CeteUserBot`")
          elif response.text.startswith("Forward"):
             await event.edit("`Gizlilik ayarları yüzenden alıntı yapamadım CeteUserBot`")
          else: 
