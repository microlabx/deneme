...
@register(pattern=".bukim(?: |$)(.*)", outgoing=True)
async def who(event):

    await event.edit(
        "`*Cete Network Zone* ' dan bazı verileri çalarken sıkı durun...`")
    ...
    try:
        ...
        event.edit("`Bu kullanıcının bilgilerini getiremedim.CeteUserBot`")
        return
...
async def fetch_info(replied_user, event):
    ...
    replied_user_profile_photos = await event.client(
        GetUserPhotosRequest(user_id=replied_user.user.id,
                             ...
                             max_id=0,
                             limit=80))
    replied_user_profile_photos_count = "Kişinin profil resmi yükleme konusunda yardıma ihtiyacı var.CeteUserBot"
    try:
    ...
    first_name = first_name.replace(
        "\u2060", "") if first_name else ("Bu kullanıcının adı yok CeteUserBot")
    last_name = last_name.replace(
        "\u2060", "") if last_name else ("Bu kullanıcının soyadı yok CeteUserBot")
    username = "@{}".format(username) if username else (
        "Bu kullanıcının kullanıcı adı yok CeteUserBot")
    user_bio = "Bu kullanıcının hakkında hiçbir şey yok CeteUserBot" if not user_bio else user_bio
...
CMD_HELP.update({
    "bukim":
    ".bukim <kullanıcı adı> veya .bukim komutu ile birinin metnine cevap verin.\
