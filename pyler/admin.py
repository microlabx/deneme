...
async def set_group_photo(gpic):
    ...
    if not gpic.is_group:
        await gpic.edit("`Bunun bir grup olduğunu sanmıyorum.CeteUserBot`")
        return
...
@register(outgoing=True, pattern="^.yetkiver(?: |$)(.*)")
@register(incoming=True, from_users=BRAIN_CHECKER, pattern="^.yetkiver(?: |$)(.*)")
async def yetkiver(promt):
    """ .yetkiver komutu ile belirlenen kişiyi yönetici yapar """
    # Hedef sohbeti almak
    ...
    await promt.edit("`Yetkilendiriliyor...CeteUserBot`")
    ...
    try:
        ...
        await promt.edit("`Başarıyla yetkilendirildi!CeteUserBot`")
...
@register(outgoing=True, pattern="^.yetkidüsür(?: |$)(.*)")
async def yetkidüsür(dmod):
    """ .yetkidüsür komutu belirlenen kişiyi yöneticilikten çıkarır """
    # Yetki kontrolü
    ...
    await dmod.edit("`Yetki düşürülüyor...CeteUserBot`")
...
async def ban(bon):
    ...
    if user.id in BRAIN_CHECKER:
        await bon.edit(
            "`Ban Hatası! Seden Yetkilisini yasaklayamam.CeteUserBot`"
        )
        return

    # Hedefi yasaklayacağınızı duyurun
    await bon.edit("`CeteUserBot Tarafından Banlandı!`")
    ...
    try:
        ...
        await bon.edit(
            "`Mesaj atma hakkım yok! Ama yine de kullanıcı yasaklandı!CeteUserBot`")
        return
    # Mesajı silin ve ardından komutun
    # incelikle yapıldığını söyleyin
    if reason:
        await bon.edit(f"`{str(user.id)}` yasaklandı CeteUserBot !!\nNedeni: {reason}")
    else:
        await bon.edit(f"`{str(user.id)}` yasaklandı CeteUserBot !!")
...
async def nothanos(unbon):
    ...
    await unbon.edit("`Yasak kaldırılıyor...CeteUserBot`")
    ...
    try:
        ...
        await unbon.edit("```Yasaklama başarıyla kaldırıldı CeteUserBot```")
        ...
        await unbon.edit("`Sanırım bu kişi yasaklama mantığım ile uyuşmuyor CeteUserBot`")
...
async def spider(spdr):
    ...
    if user.id in BRAIN_CHECKER:
        await spdr.edit(
            "`Mute Hatası! Seden Yetkilisini susturamam. CeteUserBot`"
        )
        return
    ...
    if user.id == self_user.id:
        await spdr.edit(
            "`Üzgünüm ama kendime sessize alamam... CeteUserBot \n(ヘ･_･)ヘ┳━┳`")
        return

    # Hedefi sustaracağınızı duyurun
    await spdr.edit("`Sessize alınıyor...CeteUserBot`")
    if mute(spdr.chat_id, user.id) is False:
        return await spdr.edit('`Hata! Kullanıcı zaten sessize alındı. CeteUserBot`')
    else:
        try:
            await spdr.client(
            ...
            return await spdr.edit("`Sanırım bu kişi sessize alma mantığım ile uyuşmuyor CeteUserBot`")
...
async def mutmsg(spdr, user, reason):
    ...
    if reason:
        ...
        await spdr.edit("`Kullanıcı sessize alındı !!CeteUserBot`")
...
@register(outgoing=True, pattern="^.konus(?: |$)(.*)")
async def unmoot(unmot):
    """ .konus komutu belirlenin kişinin sesini açar (yani grupta tekrardan konuşabilir) """
    # Yetki kontrolü
    ...
    await unmot.edit('```Sessizden çıkarılıyor...CeteUserBot```')
    ...
    if unmute(unmot.chat_id, user.id) is False:
        return await unmot.edit("`Hata! Kullanıcı zaten sessizden çıkarıldı.CeteUserBot`")
    else:

        try:
            await unmot.client(
                EditBannedRequest(unmot.chat_id, user.id, UNBAN_RIGHTS))
            await unmot.edit("`Kullanıcı tekrardan konuşabilir!CeteUserBot`")
        except UserAdminInvalidError:
            await unmot.edit("`Kullanıcı tekrardan konuşabilir!CeteUserBot`")
        except:
            await unmot.edit("`Sanırım bu kişi sessizden çıkarma mantığım ile uyuşmuyor CeteUserBot`")
            return
...
async def ungmoot(un_gmute):
    ...
    await un_gmute.edit('```Küresel susturma kaldırılıyor...CeteUserBot```')

    if ungmute(user.id) is False:
        await un_gmute.edit("`Hata! Muhtemelen kullanıcının kısıtlanması yok. CeteUserBot`")
    else:
        # Başarı olursa bilgi ver
        await un_gmute.edit("```Kullanıcı tekradan konuşabilir CeteUserBot```")
...
async def gspider(gspdr):
    ...
    # Eğer kullanıcı sudo ise
    if user.id in BRAIN_CHECKER:
        await gspdr.edit("`Gmute Hatası! Seden Yetkilisini küresel olarak susturamam.CeteUserBot`")
        return

    # Başarı olursa bilgi ver
    await gspdr.edit("`Susturuluyor...CeteUserBot`")
    if gmute(user.id) is False:
        await gspdr.edit(
            '`Hata! Kullanıcı zaten küresel olarak susturuldu.CeteUserBot`')
    else:
        if reason:
            await gspdr.edit(f"`Kullanıcı küresel olarak susturuldu!CeteUserBot`Nedeni: {reason}")
        else:
            await gspdr.edit("`Kullanıcı küresel olarak susturuldu!CeteUserBot`")
...
async def rm_deletedacc(show):
    ...
    del_u = 0
    del_status = "`Silinmiş hesap bulunamadı, grup temiz CeteUserBot`"

    if con != "clean":
        await show.edit("`hayalet / silinmiş / zombi hesaplar aranıyor...CeteUserBot`")
    ...
    if not admin and not creator:
        await show.edit("`Yönetici değilim! CeteUserBot`")
        return

    await show.edit("`Silinmiş hesaplar çıkarılıyor...CeteUserBot`")
    del_u = 0
    del_a = 0
    ...
    async for user in show.client.iter_participants(show.chat_id):
        if user.deleted:
            try:
                ...
                await show.edit("`Bu grupta ban yetkim bulunmamakta CeteUserBot`")
                return
            await show.client(
...
async def pin(msg):
    ...
    if not to_pin:
        await msg.edit("`Sabitlemek için herhangi bir mesaja yanıt verin CeteUserBot`")
        return
    ...
    await msg.edit("`Başarıyla sabitlendi!CeteUserBot`")
...
async def kick(usr):
    ...
    if not user:
        await usr.edit("`Kullanıcı bulunamadı.CeteUserBot`")
        return

    # Eğer kullanıcı sudo ise
    if user.id in BRAIN_CHECKER:
        await usr.edit(
            "`Kick Hatası! Seden Yetkilisini gruptan tekmeleyemem.CeteUserBot`"
        )
        return

    await usr.edit("`Çıkartılıyor...CeteUserBot`")
    ...
    if reason:
        await usr.edit(
            f"[{user.first_name}](tg://user?id={user.id}) `gruptan atıldı CeteUserBot!`\nNedeni: {reason}"
        )
    else:
        await usr.edit(
            f"[{user.first_name}](tg://user?id={user.id}) `gruptan atıldı CeteUserBot!`")
...
async def get_users(show):
    ...
    info = await show.client.get_entity(show.chat_id)
    title = info.title if info.title else "this chat"
    mentions = '{} grubunda bulunan kişiler CeteUserBot: \n'.format(title)
    try:
    ...
    try:
        ...
        await show.edit(
            "Lanet olsun, bu büyük bir grup. Kullanıcı listesini dosya olarak gönderiyorum.CeteUserBot")
        ...
        await show.client.send_file(
            show.chat_id,
            "userslist.txt",
            caption='{} grubundaki kişiler CeteUserBot'.format(title),
            reply_to=show.id,
        )
...
async def get_user_from_event(event):
    ...
    if event.reply_to_msg_id and not len(args) == 2:
        ...
        if not user:
            await event.edit("`Kişinin kullanıcı adını, ID'sini veya yanıtını iletin!CeteUserBot`")
            return
...
async def get_bots(show):
    ...
    try:
        ...
        await show.client.send_file(
            show.chat_id,
            "botlist.txt",
            caption='{} grubunda bulunan botlar CeteUserBot:'.format(title),
            reply_to=show.id,
        )
        remove("botlist.txt")
